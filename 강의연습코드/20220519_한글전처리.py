...
class 한국어_전처리기:
    ...
    def 전처리_결과_출력(self,n=1):
        ...
        for i in tqdm(self.data['clean_X']):
            ...
            self.data['y_name'] = self.data['y']
        self.data['encoder_y'] = LabelEncoder().fit_transform(self.data['y'])
        self.data['categorical_y'] = list(to_categorical(self.data['encoder_y']))
        if n==1:
            Y = np.array(self.data['encoder_y'])
        else:
            Y = to_categorical(self.data['encoder_y'])
        x_data, test_x, y_data, test_y = train_test_split(X_data, Y, test_size=0.3, random_state=0)
